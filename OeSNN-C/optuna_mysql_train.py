...
def objective(trial):
    ...
    if unfired_trials > 10 \
        or np.amax(np.array(SNN.GetFiredLog())) < 2 \
        or np.amax(np.array(SNN.GetFiredLog())) > trainSet.shape[0]/4: \
        return sum(trainSet[:,0])**2 + unfired_trials**2 + np.amax(np.array(SNN.GetFiredLog()))**2
    else: return mse
