...
OeSNN::OeSNN(int Wsize, int NOsize, int NIsize, double TS, double sim,  double C, double mod,
    ...
    _hyperParam = {
        ...
        .anomalyClassificationFactor = (anomalyFactor >= 2 ? anomalyFactor : 2)
    };
}
...
bool OeSNN::ClassifyAnomaly() {
    vector<double> eVec;
    for (int k = _E.size() - 2; k > _E.size() - 2 - (_hyperParam.Wsize - 1) && k >= 0; k--) {
        if (_G[k] == false) eVec.push_back(_E[k]);
    }

    return eVec.size() > 0 ? ( ( _E[_E.size() - 1] - CalculateAvg(eVec) )
        > _hyperParam.anomalyClassificationFactor * CalculateStd(eVec) ) : false;
}
